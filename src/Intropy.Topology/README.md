...
- **Edges** — the topology is only the wiring *between* components: async topics (`Publishes`/`Subscribes`), sync APIs (`Provides`/`Consumes`), external connectors (`From`/`To`), and service uses. Runtime activation/workload shape is not modeled — it lives in the component's scaffold.
