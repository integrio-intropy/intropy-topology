...
Each `Add*` call on `SystemBuilder` returns the block's builder directly, whose members are exactly that block's legal edges. A method that would be illegal for the block type does not exist on its builder type, so the compiler rejects it before validation ever runs. The topology records only the edges *between* components — how a component is activated at runtime (a schedule vs. a long-running app) and its workload shape are not modeled; they live in the component's own scaffold.
