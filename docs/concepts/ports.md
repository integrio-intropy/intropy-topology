...
> A port is the named port an edge block reaches the outside world through; it materializes as exactly one Dapr binding component whose name is derived from the port's name.
...
Every edge block reaches the outside world through a port. All connectivity runs through Dapr — a port never bypasses it. Each port becomes exactly one Dapr binding component whose name is derived from the port's name (`binding.<port-name>`), never declared.
...
Each used port folds into a `PortResource` carrying the derived Dapr component name, the union of used directions, and the components using it:
